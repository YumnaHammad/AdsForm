...
export async function getCurrentEntry() {
  ...
  console.log('[DB] Found existing entry:', { _id: entry._id, is_complete: entry.is_complete });
  return entry;
}
...
export async function updateField(fieldName, value, updatedBy) {
  ...
  console.log('[DB] Updating MongoDB document:', { entryId: entry._id, fieldName, value, updatedBy });
...
export async function submitForm() {
  ...
  const entry = await getCurrentEntry();
  console.log('[DB] Current entry to submit:', { _id: entry._id, is_complete: entry.is_complete });
...
export async function getAllSubmittedRecords() {
  ...
  const records = await FormEntry.find()
    .sort({ updatedAt: -1 })

  
  console.log('[DB] Found records from MongoDB:', records.length);
  console.log('[DB] Record IDs:', records.map(r => r._id));
