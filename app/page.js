...
export default function Home() {
        ...
        fields.forEach((field) => {
          ...
          if (pendingFields.has(fieldKey) || !savedFields.has(fieldKey)) {
            if (localValue.trim() !== "") {
              ...
              console.log(
                "[FORM] Preserving unsaved local change for:",
                fieldKey,
                localValue
              );
            }
  ...
  useEffect(() => {
    ...
    if (missing.length > 0) {
      console.log(
        "[FORM] Missing fields preventing submission:",
        missing.map((f) => `${f.label} (${f.key}): "${f.value}"`)
      );
    } else {
      console.log(
        "[FORM] ✅ All fields filled! Submit button should be enabled."
      );
    }
  }, [formData]);
  ...
  const handleFieldChange = (fieldName, value) => {
    console.log("[FORM] Field change detected (local only, NOT saved to DB):", {
      fieldName,
      value,
    });
    ...
    console.log("[FORM] Updating local state only (NOT in database):", {
      fieldName,
      value,
    });
  ...
  const handleSaveField = async (fieldName, value) => {
    ...
    try {
      console.log("[FORM] Saving field to MongoDB:", {
        fieldName,
        value,
      ...
      if (result.success) {
        console.log("[FORM] Successfully saved to MongoDB:", {
          fieldName,
          value,
        });
        ...
        setStatusMessage({
          type: "success",
          text: `${
            fields.find((f) => f.key === fieldName)?.label
          } saved successfully!`,
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("/api/form", {
        ...
        body: JSON.stringify({
          action: "complete", // Changed from 'submit' to 'complete'
        }),
      });
      ...
      if (result.success) {
        ...
        // DO NOT clear the form data here - only reset local state for new entry
        const emptyFormData = {
          initiated_by: "",
          product: "",
          agent_name: "",
        ...
        console.log("[FORM] Resetting local form state for new entry...");
        ...
        // DO NOT call clear action on the database
        // The database should keep the current document with is_complete: true
        // and automatically create a new empty document for the next form
              ...
              <div className="input-with-save">
                <input
                  ...
                  className={`form-input ${
                    fieldErrors[field.key] ? "form-input-error" : ""
                  }`}
